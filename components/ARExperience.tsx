  ...
  useEffect(() => {
    ...
    if ('xr' in navigator && navigator.xr) {
      navigator.xr.isSessionSupported('immersive-ar').then((supported: boolean) => {
        ...
        if (!supported) {
          setSupportInfo('Your browser supports WebXR but not AR mode. Try using Chrome on Android or Safari on iOS.')
        }
      }).catch((error) => {
        setSupportInfo(`Error checking AR support: ${error.message}`)
      })
    } else {
      setSupportInfo('WebXR is not available in your browser. Please use a WebXR-compatible browser.')
    }
  }, [])

  const initializeAR = async () => {
    if (!canvasRef.current || !isSupported) {
      setError('AR is not supported or canvas not ready')
      return
    }
        ...
        <div className="flex flex-col items-center justify-center h-full p-4">
          ...
          {isSupported ? (
            ...
            <div className="text-center">
              <p className="text-red-500 mb-4">WebXR AR is not supported on this device</p>
              <p className="text-sm text-gray-600 max-w-md">{supportInfo}</p>
              <div className="mt-6 p-4 bg-gray-100 rounded-lg text-left">
                <h3 className="font-semibold mb-2">Requirements for WebXR AR:</h3>
              ...
              <button
                onClick={() => window.location.href = 'https://immersiveweb.dev/'}
